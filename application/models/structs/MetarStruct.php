...
class MetarStruct
{
	...
	static function import(stdClass $metar): static {
		...
		$struct->visib = (string)$metar->visib ?? null;
		$struct->altim = (float)$metar->altim ?? null;
		$struct->slp = (float)$metar->slp ?? null;
		$struct->qcField = $metar->qcField ?? null;
		$struct->metarType = $metar->metarType ?? null;
		$struct->rawOb = $metar->rawOb ?? null;
		$struct->lat = (float)$metar->lat ?? null;
		$struct->lon = (float)$metar->lon ?? null;
		$struct->elev = (int)$metar->elev ?? null;
